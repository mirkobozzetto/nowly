---
title: 1.0.0
description: The first stable launch of Nowly, covering the browser extension, native host, public library, and publishing workflow.
---
...
<ReleaseTable
  ...
  rows={[
    ["Browser extension", "Detects supported websites, loads signed presences, and forwards activity to the native host.", "This is the user-facing entry point for all presence activity."],
    ["Native host", "Bridges browser activity into Discord Rich Presence.", "Discord updates happen locally without turning the browser into the integration layer."],
...
The extension is the first visible piece of the system. It watches supported sites, installs the right presence bundle, and keeps activity moving to the native host without asking the user to manage the plumbing.

- Presences only run on matched websites.
- The extension loads the correct signed bundle for the presence.
- Activity is forwarded to Discord through the local native host.
- User script support is documented and gated because it is a sensitive permission.

## Native host

The native companion is what turns page state into an actual Discord status. It keeps the browser layer focused and gives the system a clean handoff point.
...
- The onboarding flow explains the relationship between extension, native host, and presences.
