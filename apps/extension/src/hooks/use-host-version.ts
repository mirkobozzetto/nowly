...
const getNativeStatusAfterOptionalRestart = async (restartNative: boolean): Promise<NativeStatus | undefined> => {
  if (!restartNative) return sendMessage<NativeStatus>("GET_NATIVE_STATUS");

  await sendMessage<NativeStatus>("RESTART_NATIVE");
  for (let attempt = 0; attempt < 6; attempt += 1) {
    await delay(250);
    const status = await sendMessage<NativeStatus>("GET_NATIVE_STATUS");
    if (status?.version) return status;
  }

  return sendMessage<NativeStatus>("GET_NATIVE_STATUS");
};
